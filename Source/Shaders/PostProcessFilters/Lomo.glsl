...
// Custom uniforms
uniform float temperatureGamma;
uniform float colorTemperatureBottom;
uniform float colorTemperatureTop;
uniform float vignetteAmount;
uniform float vignetteGamma;
uniform float saturation;
uniform float finalMultiplier;

// Example custom uniform values...
// temperatureGamma = 0.6
// colorTemperatureBottom = 3000.0
// colorTemperatureTop = 15000.0
// vignetteAmount = 1.0
// vignetteGamma = 4.0
// saturation = 0.7
// finalMultiplier = 1.3
...
vec3 colorTemperatureFilter(vec3 rgb, float colorTemperature)
{
    ...
    float ct = mix(colorTemperatureTop, colorTemperatureBottom, pow(1.0-v_textureCoordinates.y, temperatureGamma));
    vec3 temperatureRgb =  rgbFromTemperature( ct );
...
void main(void)
{
    ...
    vec3 sRGB = texture2D(u_texture, v_textureCoordinates).rgb;
    ...
    rgb = lomoGradeFilter(rgb, saturation, colorTemperatureTop, finalMultiplier);
